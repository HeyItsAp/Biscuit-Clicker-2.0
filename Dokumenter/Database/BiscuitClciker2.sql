...
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `biscuitclicker2` ;
USE `biscuitclicker2` ;
...
CREATE TABLE IF NOT EXISTS `biscuitclicker2`.`user` (
  ...
  `auto_saving` TINYINT(4) NOT NULL DEFAULT 1,
  PRIMARY KEY (`id_user`),
...
CREATE TABLE IF NOT EXISTS `biscuitclicker2`.`biscuit_progress` (
  ...
  `id_foregin_user` INT(11) NOT NULL,
  `biscuit_count` BIGINT(20) NOT NULL DEFAULT 0,
  `prestige_count` BIGINT(20) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id_progress`),
  INDEX `fk_biscuit_progress_user1_idx` (`id_foregin_user` ASC),
...
CREATE TABLE IF NOT EXISTS `biscuitclicker2`.`upgrades` (
  `id_upgrades` INT(11) NOT NULL AUTO_INCREMENT,
  `upgrade_navn` VARCHAR(255) NOT NULL,
  `upgrade_headline` VARCHAR(255) NOT NULL,
  `upgrade_unlocked` TINYINT(4) NOT NULL DEFAULT 0,
  `upgrade_value` BIGINT(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `biscuitclicker2`.`user_has_items` (
  `user_has_item_id` INT(11) NOT NULL,
  `item_in_question_id` INT(11) NOT NULL,
  `items_obtained` TINYINT(4) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `biscuitclicker2`.`user_has_upgrades` (
  `user_has_upgrade_id` INT(11) NOT NULL,
  `upgrade_in_question_id` INT(11) NOT NULL,
  `upgrade_antall` BIGINT(20) NOT NULL,
...
BEGIN
    ...
    SELECT id_user, NEW.id_upgrades, 0 FROM `biscuitclicker2`.`user`;
END//
...
CREATE TRIGGER trigger_new_user_biscuit_progress
AFTER INSERT ON `biscuitclicker2`.`user`
FOR EACH ROW
BEGIN
    -- Trigger body with multiple SQL statements
    INSERT INTO `biscuitclicker2`.`biscuit_progress` (`id_foregin_user`) VALUES (NEW.id_user);
END//
DELIMITER ;
-- --------------------------------------------------------
-- Data
...
INSERT INTO upgrades (upgrade_navn, upgrade_headline, upgrade_unlocked, upgrade_value, upgrade_cost, upgrade_des) VALUES ('College', 'Big step', 0, 200, 5000, "You go to Collage, your friends respect your leave and run the store <br><span class='bold-text'> Gain 200 Cookie pr second</span>");
...
UPDATE biscuit_progress SET prestige_count = 99999999 WHERE id_progress = 10;
...
GRANT SELECT, INSERT, UPDATE ON biscuitclicker2.* TO 'userClicker'@'localhost' IDENTIFIED BY 'user123';
